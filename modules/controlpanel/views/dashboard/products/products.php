...
<form class="panel_menu">
	<div class="row">
		<div class="col-lg-3">
			...
			<select class="form-control input-select" name="status">
				<option <?= ($get->get('status') == 5) ? 'selected' : '' ?> value="5">Любой статус</option>
				<option <?= ($get->get('status') == Products::STATE_ACTIVE) ? 'selected' : '' ?> value="<?= Products::STATE_ACTIVE ?>">Активно</option>
				<option <?= ($get->get('status') == Products::STATE_INACTIVE) ? 'selected' : '' ?> value="<?= Products::STATE_INACTIVE ?>">Неактивно</option>
				<option <?= ($get->get('status') == Products::STATE_REJECTED) ? 'selected' : '' ?> value="<?= Products::STATE_REJECTED ?>">Отклонено</option>
				<option <?= ($get->get('status') == Products::STATE_LOCKED) ? 'selected' : '' ?> value="<?= Products::STATE_LOCKED ?>">Заблокировано</option>
				<option <?= ($get->get('status') == Products::STATE_UNCHECKED) || !isset($_GET['status']) ? 'selected' : '' ?> value="<?= Products::STATE_UNCHECKED ?>">На проверке</option>
			</select>
		</div>
		<div class="col-lg-3">
	...
	<div class="row">
		...
		<div class="col-lg-3">
			<label for="select_name">Запчасть</label>
			<select class="form-control input-select" name="category" id="select_name"><?=$option_name ?></select>
		</div>
		<div class="col-lg-3">
...
<div class="panel_content">
	<?= (empty($products) ? '<h2 class="text-center">На данный момент объявления по заданным критериям отсутствуют</h2>' : '') ?>
	...
	<?php foreach ($products as $product) { ?>
		<li class="col-lg-12">
			...
			<div class="buttons pull-right">
				...
				<a href="javascript:void(0)" onclick="createEditModal(this.nextElementSibling,'/controlpanel/data/lockproduct/','Блокировка объявления')">Заблокировать</a>
				...
				<a href="javascript:void(0)" onclick="saveForm(this.nextElementSibling,'/controlpanel/data/unlockproduct/',this)">Разблокировать</a>
				<form class="hidden" class="add-form"><input type="hidden" name="id" value="<?= $product->id ?>"></form>
				<?php } ?>
				<?php if ($product['status'] == Products::STATE_UNCHECKED) { ?>
				<i class="fa fa-upload" onclick="window.location.href='/controlpanel/data/publishproduct/<?= $product->id ?>/'"></i>
				<i onclick="createEditModal(this.nextElementSibling,'/controlpanel/data/rejectproduct/','Отказ в публикаии')" class="fa fa-ban"></i>
				<form class="hidden" class="add-form">
					<textarea name="reason" data-title="Причина"></textarea>
					<input type="hidden" name="id" value="<?= $product->id ?>">
				</form>

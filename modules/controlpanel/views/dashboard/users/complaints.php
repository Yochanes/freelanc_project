...
<div class="panel_content">
	<?= (empty($complaints) ? '<h2 class="text-center">На данный момент жалобы по заданным критериям отсутствуют</h2>' : '') ?>
	...
	<?php foreach ($complaints as $item) { ?>
      ...
      <?=$item['text'] ?>
			<div class="buttons">
				<a href="/controlpanel/dashboard/complaints?user_id=<?= $item['user_id']; ?>">Жалобы заявителя</a>
				<a href="/controlpanel/dashboard/complaints?target_id=<?= $item['target_id']; ?>">Жалобы на получателя</a>
				<?php if ($user->id != $item['user_id']) { ?>
				<a href="javascript:void(0)" onclick="createEditModal(this.nextElementSibling,'/actions/sendntf/','Новое сообщение')">Написать заявителю</a>
				...
				<?php } ?>
				<?php if ($user->id != $item['target_id']) { ?>
				<a href="javascript:void(0)" onclick="createEditModal(this.nextElementSibling,'/actions/sendntf/','Новое сообщение')">Написать получателю</a>
				<form class="hidden" class="add-form">
					<textarea name="text" data-title="Текст сообщения"></textarea>
					<div class="file_container">
						<input type="file" name="imgs">
					...
					<input type="hidden" name="receiver_id" value="<?= $item['target_id']; ?>">
				</form>
				<?php } ?>
				<?php if ($user->id != $item['user_id']) { ?>
				<a href="javascript:void(0)" onclick="createEditModal(this.nextElementSibling,'/controlpanel/data/lockuser/','Блокировка пользователя')">Заблокировать заявителя</a>
				<form class="hidden" class="add-form">
				...
				<?php } ?>
				<?php if ($user->id != $item['target_id']) { ?>
				<a href="javascript:void(0)" onclick="createEditModal(this.nextElementSibling,'/controlpanel/data/lockuser/','Блокировка пользователя')">Заблокировать получателя</a>
				<form class="hidden" class="add-form">
					<textarea name="reason" data-title="Причина"></textarea>
					<input type="hidden" name="user_id" value="<?= $item['target_id']; ?>">
				</form>
				<?php } ?>
				<i class="fa fa-trash" onclick="deleteElement(this,this.closest('li'),<?= $item['id']; ?>,'/controlpanel/data/deletecomplaint/')"></i>
			</div>
		</li>
	<?php } ?>
	</ul>

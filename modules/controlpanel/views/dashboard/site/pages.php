...
<div class="panel_menu flex hend">
  <section>
    <button class="btn btn-success" onclick="updateUrlParams(event, this)">Обновить URL параметры</button>
    <button class="btn btn-primary" onclick="addElement(event, this)"><i class="fa fa-plus"></i> Добавить страницу
    </button>
    <form class="hidden">
      ...
      <input type="text" name="real_url" data-title="Реальный URL страницы" placeholder="URL">
  ...
  <ul class="data-list drop-down-list flex hstart vstart flex-wrap">
    <?php foreach ($items as $item) { ?>
      <li>
        ...
        <form class="hidden">
          <select name="product_group_id" data-title="Показывать товары из группы">
          <input type="text" data-title="Название страницы" name="name" value="<?= $item->name ?>">
          <input type="text" data-title="URL страницы" name="url" value="<?= $item->url ?>">
          <input type="text" data-title="Реальный URL страницы" name="real_url" value="<?= (!empty($item->real_url) ? $item->real_url : $item->url) ?>">
...
<script type="text/javascript">
  ...
  function updateUrlParams(ev, _this) {
    ev.preventDefault();
    ev.stopPropagation();
    window._http.post('/controlpanel/data/updateurlparams/', { button: _this }, {
